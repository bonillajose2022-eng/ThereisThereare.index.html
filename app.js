...
// =========================================================
// SUPABASE CONFIG
// Replace with your real credentials from supabase.com → Settings → API
// =========================================================
...
function initSupabase() {
  var urlOk = SUPABASE_URL && SUPABASE_URL.indexOf('TU-URL') === -1;
  var keyOk = SUPABASE_KEY && SUPABASE_KEY.indexOf('TU-ANON') === -1;
  if (typeof window !== 'undefined' && window.supabase && urlOk && keyOk) {
    db = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
    console.log('Supabase conectado correctamente');
  } else {
    console.info('Modo local: Supabase no configurado aun. Los datos se guardan en localStorage.');
  }
}
...
// SCORE MANAGEMENT
// =========================================================
async function updateScore(delta, exerciseId, exerciseType, correct) {
  if (delta === undefined) delta = 1;
  if (exerciseId === undefined) exerciseId = '';
  if (exerciseType === undefined) exerciseType = '';
  ...
  // Save to Supabase if configured and student is registered
  var studentId = localStorage.getItem('tita_student_id');
  if (db && studentId && exerciseId) {
    try {
      ...
      console.warn('Could not save to Supabase (local mode):', err.message);
    }
  }
}
...
async function submitQuiz(e) {
  ...
  var submitBtn = document.querySelector('#quizForm button[type="submit"]');
  if (submitBtn) {
  ...
  QUIZ_QUESTIONS.forEach(function(q, i) {
    ...
    if (q.type === 'mcq') {
      ...
      var fillInput = form.querySelector('[name="q' + i + '"]');
      userAnswer = fillInput ? fillInput.value.trim() : '';
      correct = userAnswer.toLowerCase() === q.answer.toLowerCase();
      correctAnswer = q.answer;

    } else if (q.type === 'writing') {
      var textarea = form.querySelector('[name="q' + i + '"]');
      userAnswer = textarea ? textarea.value.trim() : '';
      var hasContent = userAnswer.length > 5;
      var hasThereis = userAnswer.toLowerCase().indexOf('there is') !== -1;
      var hasThereAre = userAnswer.toLowerCase().indexOf('there are') !== -1;
      correct = hasContent && (hasThereis || hasThereAre);
  ...
  // Update score manually (avoid async timing issues)
  STATE.score += score;
  STATE.totalAnswered += 1;
  localStorage.setItem('tita_score', STATE.score);
  localStorage.setItem('tita_answered', STATE.totalAnswered);
  ...
  // Save quiz result to Supabase if configured
  var studentId = localStorage.getItem('tita_student_id');
  var urlOk = SUPABASE_URL && SUPABASE_URL.indexOf('TU-URL') === -1;
  if (db && studentId && urlOk) {
    try {
      ...
      console.warn('Could not save quiz to Supabase:', err.message);
    }
  }
  ...
  // Show results - get fresh reference to container
  var container = document.getElementById('quizContainer');
  if (container) {
    showQuizResults(results, container);
  } else {
    console.error('quizContainer not found after submission');
  }
}
...
function showQuizResults(results, container) {
  // Safety check
  if (!container) {
    console.error('showQuizResults: container is null');
    return;
  }
  if (!results || results.length === 0) {
    console.error('showQuizResults: no results to show');
    return;
  }

  var score = results.filter(function(r) { return r.correct; }).length;
  var pct   = Math.round((score / results.length) * 100);
  var emoji = pct >= 80 ? '🌟' : pct >= 60 ? '😊' : '📚';
  var msg   = pct >= 80 ? 'Excellent work!' : pct >= 60 ? 'Good job! Keep practising.' : 'Keep studying — you can do it!';
  ...
  results.forEach(function(r, i) {
    if (!r.correct) {
      ...
      if (q && q.type === 'mcq' && i < 3) wrongTopics.push('Basic There is / There are');
      if (i === 3 || i === 11)            wrongTopics.push("Short answers (Yes, there is / No, there aren't)");
      if (i === 4 || i === 7 || i === 10 || i === 12) wrongTopics.push('Fill in the blank exercises');
      if (i === 6 || i === 13)            wrongTopics.push('Prepositions of place');
      if (i === 8)                        wrongTopics.push('Common mistakes section');
    }
  });
  // Remove duplicates
  var unique = wrongTopics.filter(function(v, i, a) { return a.indexOf(v) === i; });

  // Build score badge color
  var circleColor = pct >= 80
    ? 'linear-gradient(135deg,#06D6A0,#04B488)'
    : pct >= 60
    ? 'linear-gradient(135deg,#FFD166,#FFA500)'
    : 'linear-gradient(135deg,#FF6B6B,#cc3333)';

  var html = '<div class="quiz-results">';

  // Score circle
  html += '<div class="quiz-score-circle" style="background:' + circleColor + '">';
  html += '<span class="score-num">' + score + '</span>';
  html += '<span class="score-total">/ ' + results.length + '</span>';
  html += '</div>';

  // Message
  html += '<h3 style="font-family:var(--font-title);color:var(--primary);font-size:1.6rem;margin-bottom:6px;">' + emoji + ' ' + msg + '</h3>';
  html += '<p style="color:var(--text-light);margin:4px 0 16px;">' + pct + '% correct &nbsp;|&nbsp; ' + score + ' out of ' + results.length + ' questions</p>';

  // Star rating
  var stars = pct >= 80 ? '⭐⭐⭐' : pct >= 60 ? '⭐⭐' : '⭐';
  html += '<div style="font-size:1.8rem;margin-bottom:16px;">' + stars + '</div>';

  html += '<div class="divider"></div>';

  // Results list
  html += '<div style="text-align:left;margin-top:16px;">';
  html += '<h4 style="color:var(--primary);margin-bottom:12px;font-family:var(--font-title);">📋 Your Answers:</h4>';

  results.forEach(function(r, i) {
    var rowClass = r.correct ? 'r-correct' : 'r-wrong';
    var icon     = r.correct ? '✅' : '❌';
    var answerInfo = r.correct
      ? 'Your answer: <em>' + (r.userAnswer || '(empty)') + '</em>'
      : 'Your answer: <em>' + (r.userAnswer || '(empty)') + '</em> &nbsp;→&nbsp; Correct: <strong>' + r.correctAnswer + '</strong>';

    html += '<div class="result-item ' + rowClass + '">';
    html += '<span class="result-icon">' + icon + '</span>';
    html += '<div><strong>Q' + (i + 1) + ':</strong> ' + r.question + '<br><span style="font-size:0.88rem;">' + answerInfo + '</span></div>';
    html += '</div>';
  });
  ...
  // Suggestions box
  if (unique.length > 0) {
    html += '<div class="suggestions-box" style="margin-top:20px;">';
    html += '<h4>📝 What to review:</h4>';
    html += '<ul style="list-style:none;margin-top:8px;">';
    unique.forEach(function(t) {
      html += '<li style="padding:5px 0;">→ Go back to: <strong>' + t + '</strong></li>';
    });
    html += '</ul></div>';
  } else {
    html += '<div class="grammar-box green" style="margin-top:20px;text-align:center;">';
    html += '<p>🎉 Perfect! You have mastered <strong>There is / There are</strong>!</p>';
    html += '</div>';
  }

  html += '</div>'; // close quiz-results

  // Teacher reset button (always rendered, hidden by CSS unless teacher mode)
  html += '<div class="teacher-reset-area" id="quizResetArea" style="margin-top:16px;text-align:center;' + (STATE.teacherMode ? '' : 'display:none;') + '">';
  html += '<button class="btn btn-danger btn-sm" onclick="resetQuiz()">🗑 Reset Quiz (Teacher)</button>';
  html += '</div>';
...
// =========================================================
// =========================================================
// REGISTER STUDENT (Supabase + localStorage fallback)
// =========================================================
async function registerStudent() {
  ...
  if (!name || name.length < 2) {
    errorEl.textContent = 'Please enter your name! / Escribe tu nombre';
  ...
  if (!studentId) {
    // Only try Supabase if credentials look real
    var urlOk = SUPABASE_URL && SUPABASE_URL.indexOf('TU-URL') === -1;
    var keyOk = SUPABASE_KEY && SUPABASE_KEY.indexOf('TU-ANON') === -1;
    ...
    if (db && urlOk && keyOk) {
      try {
        ...
        console.log('Student saved in Supabase:', studentId);
      } catch (err) {
        console.warn('Supabase failed, using local mode:', err.message);
        studentId = 'local-' + Date.now();
      }
    } else {
      // No Supabase configured - use local ID
      studentId = 'local-' + Date.now();
      console.info('Local mode: student ID =', studentId);
    }
  ...
  STATE.studentName = localStorage.getItem('tita_student_name') || name;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // If student already registered, skip login screen
  var savedId   = localStorage.getItem('tita_student_id');
  var savedName = localStorage.getItem('tita_student_name');
  ...
  // Flashcard click
  var fci = document.getElementById('flashcardInner');
  if (fci) fci.addEventListener('click', flipCard);

  // Quiz tab
  document.querySelector('.nav-tab[data-section="quiz"]').addEventListener('click', function() {
